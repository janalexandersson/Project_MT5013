...
#fixa så actors kan ta NA när de inte finns
...
imdb_func <- function(url){
  ...
  #runtime
  runtime <- html_nodes(webpage,'.text-primary , .text-muted .runtime')
  ...
  #actors
  nthActor <- function(webpage, number){
    actor <- html_nodes(webpage,'.text-muted+ p')
    ...
    for(i in 1:length(actor)){
      v <-append(v, actor[[i]][number])
    }
    actor <- v
  } 
  
  actor1 <- nthActor(webpage, 1)
  actor2 <- nthActor(webpage, 2)
  actor3 <- nthActor(webpage, 3)
  actor4 <- nthActor(webpage, 4)
...
runtime <- runtime2
##########################################################
#hamnade en 4:a mellan varje namn av någon anledning
...
imdb_df <- imdb_df %>% 
  ...
  mutate(actor3 = as.character(actor3)) %>% 
  mutate(actor4 = as.character(actor4)) %>% 
...
getCount(actors) %>% arrange(desc(count)) %>% slice(1:50)
########################
